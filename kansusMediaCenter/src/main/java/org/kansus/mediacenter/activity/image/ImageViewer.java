...
import org.kansus.mediacenter.util.GetterHandler;
import org.kansus.mediacenter.util.ImageGetter;
import org.kansus.mediacenter.util.ImageGetterCallback;
import org.kansus.mediacenter.util.ImageManager;
import org.kansus.mediacenter.util.RotateBitmap;
import org.kansus.mediacenter.util.Util;
import org.kansus.mediacenter.widget.ImageViewTouchBase;
...
public class ImageViewer extends Activity implements View.OnClickListener {
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		MenuItem item = menu.add(Menu.NONE, Menu.NONE,
				MenuHelper.POSITION_SLIDESHOW, R.string.slide_show);
		...
		mImageMenuRunnable = MenuHelper.addImageMenuItems(menu,
				MenuHelper.INCLUDE_ALL, ImageViewer.this, mHandler,
				mDeletePhotoRunnable, new MenuHelper.MenuInvoker() {
		...
		item.setOnMenuItemClickListener(new MenuItem.OnMenuItemClickListener() {
			public boolean onMenuItemClick(MenuItem item) {
				...
				preferences.setClass(ImageViewer.this, Settings.class);
	...
	private IImageList buildImageListFromUri(Uri uri) {
		String sortOrder = mPrefs.getString("pref_gallery_sort_key",
				"descending");
...
class ImageViewTouch extends ImageViewTouchBase {
	private final ImageViewer mViewImage;
	private boolean mEnableTrackballScroll;

	public ImageViewTouch(Context context) {
		super(context);
		mViewImage = (ImageViewer) context;
	}
		...
		try {
			...
			case KeyEvent.KEYCODE_DPAD_CENTER: {
				if (mViewImage.isPickIntent()) {
					...
					mViewImage.setResult(ImageViewer.RESULT_OK,
			...
			if (nextImagePos >= 0
				...
				synchronized (mViewImage) {
					mViewImage.setMode(ImageViewer.MODE_NORMAL);
